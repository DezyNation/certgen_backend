...
class CertificateController extends Controller
{
    ...
    public function pdf(string $id)
    {
        ...
        $form = Form::find($submission->form_id);
        $template = $submission->template();
        $qrcode = base64_encode(QrCode::format('png')->size($template->qr_dimension)->generate("https://certificate.itasinc.in/certificates/verify?cert_id={$submission->certificate_id}"));
        $data = [
            'image' => $template->path,
    ...
    public function sample(Request $request, string $id)
    {
        ...
        $qrcode = base64_encode(QrCode::format('png')->size($template->qr_dimension)->generate("https://certificate.itasinc.in/certificates/verify?cert_id=24"));
        $data = [
            'image' => $template->path,
            'qr_size' => $template->qr_dimension,
            'certificate_id' => "24",
    ...
    {
        $submission = Submission::where(['approved' => 1])->whereNotNull('form_id')->where(function ($q) use ($id) {
